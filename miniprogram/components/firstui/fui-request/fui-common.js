// 本文件由FirstUI授权予闫弘宇（手机号：1 35   1000  1 5   53，身份证尾号： 03 3 6  12）专用，请尊重知识产权，勿私下传播，违者追究法律责任。
const base = {
	//所有配置项，未设置则使用默认值
	config() {
		return {
	...
	getOptions(config) {
		...
		['host', 'timeout', 'prevent', 'keys', 'brief', 'cancelToken', 'showLoading', 'loadingText', 'errorMsg','arrayFormat']
		.forEach(item => {
			delete options[item];
		})
		return options;
